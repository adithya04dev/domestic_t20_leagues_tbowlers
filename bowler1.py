...
class Bowler():
            ...
            def create_df(self,leagues,team_name,overs,BatterType,Season):
                    ...
                    balls=len(self.df.loc[(self.df['is_wide']==0)  & (self.df['is_noball']==0) & (self.df["LeagueName"].isin(leagues)) & (self.df["BattingType"].isin(BatterType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season))  & (self.df['BowlingTeam'] == team_name) ] )
                    out = len(   self.df.loc[(self.df["BowlingTeam"] == team_name) & (self.df["LeagueName"].isin(leagues)) & (self.df["player_out"]==1)  & (self.df["BattingType"].isin(BatterType))   & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))])
                    ...
                    dots=len(self.df.loc[(self.df["BowlingTeam"] == team_name) & (self.df["LeagueName"].isin(leagues)) & (self.df["Extras_Run"]==0) & (self.df["batsman_run"]==0) & (self.df["BattingType"].isin(BatterType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season))  ] )
...
# bow=Bowler(df)
# result=bow.calculateb(['TNPL'],'Chepauk Super Gillies',[1,2,3],["RHB"],[2023])
# r1=bow.overall()
# print(r1)
# with open('C:/Users/adith/Documents/ds/t20_leagues/set_3_leagues/app/teams/bowling/bowling1.pkl', 'wb') as f:
#         pickle.dump(bow, f)
